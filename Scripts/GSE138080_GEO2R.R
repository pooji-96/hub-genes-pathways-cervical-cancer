...
gset <- getGEO("GSE138080", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
gsms <- "00000000001111111111111112222222222"
...
# tT <- subset(tT, select=c("ID","adj.P.Val","P.Value","F","GenBank_Accession","Gene.symbol","Gene.title"))
...
par(mar=c(7,4,2,1))
title <- paste ("GSE138080", "/", annotation(gset), sep ="")
...
for (i in 1:3) {
  ...
  volcanoplot(fit2, coef = i, 
              ...
              main = paste("GSE138080 -", colnames(fit2)[i]), 
              xlim = c(-7, 7), ylim = c(0, max(-log10(pval), 10)),
...
for (i in 1:3) {
  ...
  writeLines(unique_up_genes, paste0("DEG/GSE138080/GSE138080_", cts[i], "_upregulated_gene_symbols.txt"))
  writeLines(unique_down_genes, paste0("DEG/GSE138080/GSE138080_", cts[i], "_downregulated_gene_symbols.txt"))
}
